...
handed_IDs <- readRDS("/home/mahle68/ownCloud - enourani@ab.mpg.de@owncloud.gwdg.de/Work/Projects/HB_ontogeny_eobs/R_files/circling_w_LI_population.rds") %>% 
...
laterality_circling <- readRDS("laterality_index_per_8sec_burst_days_since.rds") %>% 
  ...
  filter(circling_status == "circling" & #only keep circling flight
           days_since_tagging <= 90) %>% #3 months
  as.data.frame()


#thin the data, so that the consecutive bursts are at least 18 seconds apart: 10 sec between the end of one and start of nest
circling_thinned <- laterality_circling %>% 
  group_by(individual_local_identifier) %>% 
...
#exploratory plot
ggplot(circling_thinned, aes(x = factor(laterality_bi), y = abs(cumulative_yaw_8sec))) +
  geom_boxplot() +
  labs(x = "Laterality", y = "Absolute Cumulative Yaw (8 sec)") +
  theme_minimal()

ggplot(circling_thinned, aes(x = days_since_tagging, y = abs(cumulative_yaw_8sec))) +
  geom_point() + 
...
m_difficulty <- gam(abs_cum_yaw ~ s(days_since_tagging),
                    data = circling_thinned, method = "REML")
...
m_difficulty2 <- gam(abs_cum_yaw ~ mean_pitch_scale + s(age_scale, k = 10),
                     data = laterality_circling, method = "REML")
...
laterality_circling <- laterality_circling %>% 
...
m_laterality <- glm(laterality_bi ~ scale(difficulty_ln),
                    data = circling_thinned,
                    family = "binomial")
...
m_laterality2 <- glmer(laterality_bi ~ scale(difficulty_ln) + (1 | individual_local_identifier),
                       data = circling_thinned,
                       family = "binomial")
...
#try a gam
m_laterality3 <- gam(laterality_bi ~ s(scale(difficulty_ln)) + 
                       s(individual_local_identifier, bs = "re"),
                     data = circling_thinned, method = "REML", family = "binomial")
...
#create a new dataset to use for making predictions for the interaction plot
new_data <- expand.grid(x = (1:90), #range of age
                        y = (1:400)) %>% #range of cumulative yaw
  rename(abs_cum_yaw_scale = y,
         age_scale = x) %>%
  #calculate z-scores. get the mean(center) and sd(scale) from the previous z transformation. for consistency
  mutate (abs_cum_yaw_scale = (abs_cum_yaw_scale - attr(circling_thinned[,colnames(circling_thinned) == "abs_cum_yaw_scale"],'scaled:center'))/
            attr(circling_thinned[,colnames(circling_thinned) == "abs_cum_yaw_scale"],'scaled:scale'),
          age_scale = (age_scale - attr(circling_thinned[,colnames(circling_thinned) == "age_scale"],'scaled:center'))/
            attr(circling_thinned[,colnames(circling_thinned) == "age_scale"],'scaled:scale'),
          individual_local_identifier = rep(sample(circling_thinned$individual_local_identifier, 10), nrow(.)/10), #randomly select 7 individual IDs
          individual_local_identifier2 = individual_local_identifier,
          laterality_bi = NA,
          mean_pitch_scale = attr(circling_thinned[,colnames(circling_thinned) == "mean_pitch_scale"],'scaled:center')) #set mean pitch to its mean
...
m_inla <- inla(laterality_bi ~ 1 + abs_cum_yaw_scale * age_scale + mean_pitch_scale +
                 f(individual_local_identifier, abs_cum_yaw_scale, model = "iid") +  
                 f(individual_local_identifier2, age_scale, model = "iid"),  
               data = data, family = "binomial",
               control.compute = list(cpo = TRUE),
...
m_inla <- inla(laterality_bi ~ 1 + abs_cum_yaw_scale + mean_pitch_scale +
...
preds <- data.frame(abs_cum_yaw_scale = data[is.na(data$laterality_bi) ,"abs_cum_yaw_scale"],
                    age_scale = data[is.na(data$laterality_bi) ,"age_scale"],
...
pred_r <- preds %>% 
  dplyr::select(age_scale, abs_cum_yaw_scale, preds) %>% #figure out if i need to do the transformation
  terra::rast(type = "xyz") %>%
  focal(w = 9, fun = median, na.policy = "all", na.rm = T) %>%
  as.data.frame(xy = T) %>% 
...
m2 <- glmer(laterality_bi ~ scale(abs(cumulative_yaw_8sec)) * scale(days_since_tagging) + (1 | individual_local_identifier),
            data = laterality_1sec_days,
            family = "binomial")
