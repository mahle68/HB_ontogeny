# script for exploring the directionality of thermal soaring in European honey buzzards using GPS
# Elham Nourani. 4.4.2024
...
gps <- movebank_retrieve(study_id = 2201086728, sensor_type_id = "gps", #download data for wintering 
                         entity_type = "event",  attributes = "all",
                         timestamp_start = as.POSIXct("2024-02-01 00:00:00"),
                         timestamp_end = as.POSIXct("2024-04-04 00:00:00"))

#filter for high-res bursts
...
lapply(inds_ls, function(ind){
  ...
  if(nrow(ind) > 0){
    ...
    saveRDS(bursts_smooth, file = paste0("R_files/GPS_seg_Apr24_winter/animal_", 
...
#STEP 3: distribution of turning angles during soaring -----------------------------


soaring_data <- list.files("R_files/GPS_seg_Apr24_winter", full.names = T) %>% 
  map(readRDS) %>% 
  bind_rows() %>% 
  filter(flight_clust_sm3 %in% c("shallow_circular_soaring", "circular_soaring")) %>% 
  filter(individual_local_identifier !="D321_584")


ggplot(soaring_data, aes(x = turn_angle, colour = individual_local_identifier)) + 
